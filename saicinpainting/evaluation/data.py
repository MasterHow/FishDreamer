...
def load_label(fname, mode='L', return_orig=False):
    img = np.array(Image.open(fname).convert(mode))
    if img.ndim == 3:
        img = np.transpose(img, (2, 0, 1))
    out_img = img.astype('float32') 
    out_img[out_img==255]=19
    if return_orig:
        return out_img, img
    else:
        return out_img
...
class InpaintingDataset(Dataset):
    def __init__(self, datadir, img_suffix='.jpg', pad_out_to_modulo=None, scale_factor=None):
        ...
        self.img_filenames = sorted(list(glob.glob(os.path.join(self.datadir, '**', '*leftImg8bit.png'), recursive=True)))
        self.label_filenames = sorted(list(glob.glob(os.path.join(self.datadir, '**', '*labelTrainIds.png'), recursive=True)))
    ...
    def __getitem__(self, i):
        ...
        label = load_label(self.label_filenames[i], mode='L')
        result = dict(image=image, mask=mask[None, ...],label=label[None,...])

        if self.scale_factor is not None:
            result['image'] = scale_image(result['image'], self.scale_factor)
            result['mask'] = scale_image(result['mask'], self.scale_factor, interpolation=cv2.INTER_NEAREST)
            result['label'] = scale_image(result['label'], self.scale_factor, interpolation=cv2.INTER_NEAREST)

        if self.pad_out_to_modulo is not None and self.pad_out_to_modulo > 1:
            result['unpad_to_size'] = result['image'].shape[1:]
            result['image'] = pad_img_to_modulo(result['image'], self.pad_out_to_modulo)
            result['mask'] = pad_img_to_modulo(result['mask'], self.pad_out_to_modulo)
            result['label'] = pad_img_to_modulo(result['label'], self.pad_out_to_modulo)
...
class OurInpaintingDataset(Dataset):
    ...
    def __getitem__(self, i):
        result = dict(image=load_image(self.img_filenames[i], mode='RGB'),
                      ...
                      label=load_label(self.label_filenames[i], mode='L')[None, ...]
                      )

        if self.scale_factor is not None:
            result['image'] = scale_image(result['image'], self.scale_factor)
            result['mask'] = scale_image(result['mask'], self.scale_factor)
            result['label'] = scale_image(result['label'], self.scale_factor)
...
class PrecomputedInpaintingResultsDataset(InpaintingDataset):
    def __init__(self, datadir, predictdir, inpainted_suffix='_inpainted.jpg', **kwargs):
        ...
        self.inpainted_filenames = sorted(list(glob.glob(os.path.join(self.predictdir, '**', '*mask.png'), recursive=True)))
        self.segmented_filenames = sorted(list(glob.glob(os.path.join(self.predictdir, '**', '*labelTrainIds.png'), recursive=True)))
    ...
    def __getitem__(self, i):
        ...
        result['inpainted'] = load_image(self.inpainted_filenames[i])
        result['segmented'] = load_label(self.segmented_filenames[i])[None,...]
   
        if self.pad_out_to_modulo is not None and self.pad_out_to_modulo > 1:
            result['inpainted'] = pad_img_to_modulo(result['inpainted'], self.pad_out_to_modulo)
            result['segmented'] = pad_img_to_modulo(result['segmented'], self.pad_out_to_modulo)
        return result
...
class OurPrecomputedInpaintingResultsDataset(OurInpaintingDataset):
    def __init__(self, datadir, predictdir, inpainted_suffix="png", **kwargs):
        ...
        # self.pred_filenames = [os.path.join(predictdir, os.path.splitext(fname[len(datadir):])[0] + inpainted_suffix)
        #                        for fname in self.mask_filenames]
        self.inpainted_filenames = sorted(list(glob.glob(os.path.join(self.datadir, '**', '*mask.png'), recursive=True)))
        self.segmented_filenames = sorted(list(glob.glob(os.path.join(self.datadir, '**', '*!!!!!!!!!!!'), recursive=True)))


    def __getitem__(self, i):
        result = super().__getitem__(i)
        result['inpainted'] = self.file_loader(self.inpainted_filenames[i])
        result['segmented'] = self.file_loader(self.segmented_filenames[i])        
...
class InpaintingEvalOnlineDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        lab = load_label(self.label_filenames[i], mode='L', return_orig=False)
        mask = self.mask_generator(img, raw_image=raw_image)
        result = dict(image=img, mask=mask,label=lab[None,...])
