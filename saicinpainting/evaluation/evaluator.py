...
class InpaintingEvaluator():
    def __init__(self, dataset, scores, area_grouping=False, bins=10, batch_size=32, device='cuda',
                 integral_func=None, integral_title=None, clamp_image_range=None):

        LOGGER.info('######inpainting evalutaor called')
        """
        ...
        for score_name, score in tqdm.auto.tqdm(self.scores.items(), desc='scores'):
            ...
            with torch.no_grad():
                ...
                for batch in tqdm.auto.tqdm(self.dataloader, desc=score_name, leave=False):
                    ...
                    image_batch, mask_batch, label_batch = batch['image'], batch['mask'], batch['label']
                    ...
                    if model is None:
                        ...
                        segmented_batch = batch['segmented']#.max(1)[1].unsqueeze(1).data
                    else:
                        # print('!!!!!!!!!!!!!!!1')

                        ####### the segmented batch hier should be [N,1,H,W]??????????
                        inpainted_batch,segmented_batch = model(image_batch, mask_batch,label_batch)
                        # batch= model(batch)
                    # print('!!!!!segmented',segmented_batch.size())
                    score(inpainted_batch, image_batch, segmented_batch, label_batch, mask_batch)
                    # score(batch['inpainted'],batch['image'],batch['segmented'].max(1)[1].unsqueeze(1).data,batch['label'],batch)
...
def ssim_psnr100_f1(metrics,psnr_scale=100):
    ssim = metrics[('ssim', 'total')]['mean']
    psnr = metrics[('psnr', 'total')]['mean']

    psnr_rel=psnr/psnr_scale
    f1 = 2*ssim*psnr_rel/(ssim+psnr_rel+1e-3)
    return f1


def ssim_psnr100_miou_f1(metrics,psnr_scale=100):
    ssim = metrics[('ssim', 'total')]['mean']
    psnr = metrics[('psnr', 'total')]['mean']
    miou = metrics[('miou', 'total')]['mean']

    psnr_rel=psnr/psnr_scale
    f1 = 2*ssim*psnr_rel*miou/(ssim+psnr_rel+miou+1e-3)
    return f1


class InpaintingEvaluatorOnline(nn.Module):
    def __init__(self, scores, bins=10, image_key='image',label_key='label', inpainted_key='inpainted',segmented_key='segmented',
                 integral_func=None, integral_title=None, clamp_image_range=None):

        # LOGGER.info('$$$$$$$$$$online inpaintig evaliate called')
        """
        :param scores: dict {score_name: EvaluatorScore object}
        :param bins: number of groups, partition is generated by np.linspace(0., 1., bins + 1)
        :param device: device to use
        ...
        self.segmented_key = segmented_key
        self.bins_num = bins
    ...
    def forward(self, batch: Dict[str, torch.Tensor]):
        ...
        result = {}
        with torch.no_grad():
            image_batch,label_batch, mask_batch, inpainted_batch,segmented_batch = batch[self.image_key],batch[self.label_key] ,batch['mask'], batch[self.inpainted_key],batch[self.segmented_key].max(1)[1].unsqueeze(1).data
            segmented_batch[label_batch==19]=19
            if self.clamp_image_range is not None:
                image_batch = torch.clamp(image_batch,
                                          min=self.clamp_image_range[0],
                                          max=self.clamp_image_range[1])
            ...
            for score_name, score in self.scores.items():
                result[score_name] = score(inpainted_batch,image_batch,segmented_batch,label_batch,mask_batch)
        return result
