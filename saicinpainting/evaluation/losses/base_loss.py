...
class EvaluatorScore(nn.Module):
    @abstractmethod
    def forward(self, pred_batch, target_batch,pred_label_batch,target_label_batch, mask):
        pass
...
# def caculate_psnr(self, p0, p1, peak=255.):
...
#         batch_values = self.calculate_pnsr(pred_batch, target_batch)
...
class SSIMScore(PairwiseScore):
    ...
    def forward(self, pred_batch, target_batch,pred_label_batch=None, target_label_batch=None, mask=None):
...
class PSNRScore(EvaluatorScore):
    def __init__(self):

        LOGGER.info("PSNR score")
        super().__init__()
        self.reset()
        # self.batch_values=None

    def caculate_psnr(self, p0, p1, peak=255.):
        return 10 * torch.log10(peak ** 2 / torch.mean((255. * p0 - 255. * p1) ** 2))
    ...
    def forward(self, pred_batch, target_batch,pred_label_batch=None, target_label_batch=None, mask=None):
        length = len(pred_batch)

        batch_values = 0
        for i in range(0,length):
           
            # LOGGER.info(f'********psnr{ self.caculate_psnr(pred_batch[i], target_batch[i],255.)}')
            # LOGGER.info(f'**********{torch.unique(pred_batch[i])}')
            # LOGGER.info(f'**********{torch.unique(target_batch[i])}')
            
            batch_values=  batch_values + self.caculate_psnr(pred_batch[i], target_batch[i],255.)
           
        mean_batch = batch_values/length
       
        self.batch_values = np.hstack([
            self.batch_values, mean_batch.detach().cpu().numpy()
        ])
    ...
    def get_value(self,groups=None,states=None):
        # self.batch_values = self.batch_values.detach().cpu().numpy()
        # print("self.vatcg value shape",self.batch_values.shape)
        
        # print("self.batch_values",self.batch_values.mean())

        # LOGGER.info(f'!!!!!!!psnr{self.batch_values.mean().type()}')
        
        total_results = dict(mean=self.batch_values.mean())
...
class mIoUScore(EvaluatorScore):

 
    def __init__(self,nClasses=20, ignoreIndex=19):

        LOGGER.info("mIoU score")
        super().__init__()
    ...
    def forward(self, pred_batch, target_batch,pred_label_batch, target_label_batch, mask=None):
        target_label_batch = target_label_batch.long()
        pred_label_batch = pred_label_batch.long()

        # LOGGER.info(f'*****************pred_label{pred_label_batch.size()}')
        # LOGGER.info(f'*****************target_label{target_label_batch.size()}')
      


        # target_label_batch = target_label_batch + 1
        # target_label_batch[target_label_batch==256]=0

        # pred_label_batch = pred_label_batch +1 
        # target_label_batch[target_label_batch==255]=19

        # LOGGER.info(f'&&&&&&&&&&&&&pred_label{torch.unique(pred_label_batch)}')
        # LOGGER.info(f'&&&&&&&&&&&&&target_label{torch.unique(target_label_batch)}')
        
        if (pred_label_batch.is_cuda or target_label_batch.is_cuda):
            pred_label_batch = pred_label_batch.cuda()
            target_label_batch = target_label_batch.cuda()

        #if size is "batch_size x 1 x H x W" scatter to onehot
        if (pred_label_batch.size(1) == 1):
            
            x_onehot = torch.zeros(pred_label_batch.size(0), self.nClasses, pred_label_batch.size(2), pred_label_batch.size(3))  
            if pred_label_batch.is_cuda:
                x_onehot = x_onehot.cuda()
            x_onehot.scatter_(1, pred_label_batch, 1).float()
        else:
            x_onehot = pred_label_batch.float()
        # LOGGER.info(f'*****************x_onehot{x_onehot.size()}')

        if (target_label_batch.size(1) == 1):
            y_onehot = torch.zeros(target_label_batch.size(0), self.nClasses, target_label_batch.size(2), target_label_batch.size(3))
            if target_label_batch.is_cuda:
                y_onehot = y_onehot.cuda()
            y_onehot.scatter_(1, target_label_batch, 1).float()
        else:
            y_onehot = target_label_batch.float()
        # LOGGER.info(f'*****************y_onehot{y_onehot.size()}')
    ...
    #print(y_onehot.size())
        ...
        # LOGGER.info(f'#####tpmult{tpmult.size()}')
        tp = torch.sum(torch.sum(torch.sum(tpmult, dim=0, keepdim=True), dim=2, keepdim=True), dim=3, keepdim=True).squeeze()
        # LOGGER.info(f'#####tp{tp.size()}')
    ...
    def get_value(self,groups=None,states=None):
        ...
        iou = num / den
        # LOGGER.info(f'!!!!!!!miou{iou.type()}')

        
        # LOGGER.info(f'!!!!!!!miou{np.dtype(torch.mean(iou).detach().cpu().numpy())}')

        total_results = dict(mean=torch.mean(iou).detach().cpu().numpy())
