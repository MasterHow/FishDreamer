...
class InpaintingTrainDataset(Dataset):
    def __init__(self, indir, mask_generator, transform):
        self.in_files = sorted(list(glob.glob(os.path.join(indir, '**', '*leftImg8bit.png'), recursive=True)))
        self.in_files_label = sorted(list(glob.glob(os.path.join(indir, '**', '*labelTrainIds.png'), recursive=True)))
    ...
    def __getitem__(self, item):
        ...
        label = load_image(self.in_files_label[item],mode='L')
        label[label==255]=19
        mask = self.mask_generator(img, iter_i=self.iter_i)

        self.iter_i = self.iter_i+ 1
        result = dict(image=img, mask=mask,label=label[None,...])
        return result
