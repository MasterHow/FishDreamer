...
from saicinpainting.training.losses.cross_entropy_loss import cross_entropy
...
def make_constant_area_crop_batch(batch, **kwargs):
    ...
    batch['label'] = batch['label'][:, :, crop_y: crop_y + crop_height, crop_x: crop_x + crop_width]
    return batch


def mask_with_corner(mask_size, corner_size):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    mask = np.ones((mask_size, mask_size), np.float32)
    
    for i in range(0, corner_size):

        mask[i, 0:(corner_size-i)] = 0
        mask[i, (mask_size-(corner_size-i)):mask_size] = 0
        
    for j in range(mask_size-corner_size, mask_size):

         mask[j, 0:j-(mask_size-corner_size)+1] = 0
         mask[j, mask_size-((j-(mask_size-corner_size))+1):mask_size] = 0
   
    return torch.from_numpy(mask[None, ...]).float().to(device)


def mask_with_corner_for_label(mask_size, corner_size):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    mask = np.ones((mask_size, mask_size), np.float32)
    
    for i in range(0, corner_size):

        mask[i, 0:(corner_size-i)] = 255
        mask[i, (mask_size-(corner_size-i)):mask_size] = 255
        
    for j in range(mask_size-corner_size, mask_size):

         mask[j, 0:j-(mask_size-corner_size)+1] = 255
         mask[j, mask_size-((j-(mask_size-corner_size))+1):mask_size] = 255
...
class DefaultInpaintingTrainingModule(BaseInpaintingTrainingModule):
    ...
    def forward(self, batch):
        ...
        if self.training and self.rescale_size_getter is not None:
            ...
            batch['label']= F.interpolate(batch['label'], size=cur_size, mode='nearest')
        ...
        if self.add_noise_kwargs is not None:
            ...
            if self.noise_fill_hole:
                ...
                masked_label = masked_label + mask * noise[:, :masked_img.shape[1]]
            masked_img = torch.cat([masked_img, noise], dim=1)
            masked_label = torch.cat([masked_label, noise], dim=1)

        if self.concat_mask:
           masked_img = torch.cat([masked_img, mask], dim=1)

        batch['predicted_image'], batch['predicted_segmentation'] = self.generator(masked_img)
       
        batch['inpainted'] = mask * batch['predicted_image']+ (1 - mask) * batch['image']

        batch['segmented'] = batch['predicted_segmentation']
    ...
    def generator_loss(self, batch):
        summ = torch.sum(batch['image'], axis=0)
        corner_size = torch.nonzero(summ[0, :])[0][0]
        extra_mask = mask_with_corner(batch['image'].shape[2], corner_size)

        img = batch['image']*extra_mask 
        predicted_img = batch[self.image_to_discriminator]*extra_mask

        label=torch.squeeze(batch['label'],1).long()
        predicted_label=batch['segmented']
        ...
        cross_entropy_value = cross_entropy(predicted_label,label)*self.config.losses.cross_entropy.weight
        total_loss = total_loss + cross_entropy_value
        metrics['cross_entropy'] = cross_entropy_value

        if self.config.losses.lovasz_loss.weight > 0:
            lovasz_value = self.loss_lovasz(predicted_label, label )
            total_loss = total_loss + lovasz_value*self.config.losses.lovasz_loss.weight
            metrics['lovasz'] = lovasz_value
    ...
    def discriminator_loss(self, batch):
        ...
        real_batch = batch['image']*extra_mask
        predicted_img = batch[self.image_to_discriminator]*extra_mask.detach()

        self.adversarial_loss.pre_discriminator_step(real_batch=real_batch, fake_batch=predicted_img,
                                                     generator=self.generator, discriminator=self.discriminator)
        discr_real_pred, discr_real_features = self.discriminator(real_batch)
        discr_fake_pred, discr_fake_features = self.discriminator(predicted_img)
        adv_discr_loss, adv_metrics = self.adversarial_loss.discriminator_loss(real_batch=real_batch,
                                                                               fake_batch=predicted_img,
                                                                               discr_real_pred=discr_real_pred,
                                                                               discr_fake_pred=discr_fake_pred,
                                                                               mask=batch['mask'])
        ...
        if batch.get('use_fake_fakes', False):
            fake_fakes = batch['fake_fakes']*extra_mask
            self.adversarial_loss.pre_discriminator_step(real_batch=real_batch, fake_batch=fake_fakes,
                                                         generator=self.generator, discriminator=self.discriminator)
