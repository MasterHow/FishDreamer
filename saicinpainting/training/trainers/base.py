...
from saicinpainting.training.losses.lovasz_loss import LovaszLoss
from saicinpainting.training.losses.perceptual import PerceptualLoss, ResNetPL
from saicinpainting.training.modules import make_discriminator, make_encoder_decoder
...
def update_running_average(result: nn.Module, new_iterate_model: nn.Module, decay=0.999):   # not called
    # def update_running_average(result: nn.Module, new_iterate_model: nn.Module, decay=0.990):
    with torch.no_grad():
...
class BaseInpaintingTrainingModule(ptl.LightningModule):
    def __init__(self, config, use_ddp, *args,  predict_only=False, visualize_each_iters=100,
        ...
        self.generator = make_encoder_decoder(config, **self.config.encoder_decoder)
        ...
        if not predict_only:
            ...
            self.val_evaluator = make_evaluator(device=device, **self.config.evaluator)
            self.test_evaluator = make_evaluator(device=device, **self.config.evaluator)
            ...
            if self.config.losses.get("lovasz_loss", {"weight": 0})['weight'] > 0:
                self.loss_lovasz = LovaszLoss()
    ...
    def val_dataloader(self):
        ...
        extra_val = self.config.data.get('extra_val', ())
        if extra_val:
            res = res + [make_default_val_dataloader(**extra_val[k]) for k in self.extra_val_titles]
    ...
    def training_step_end(self, batch_parts_outputs):
        ...
        self.log(f'train_full_loss', full_loss, on_step=False,on_epoch=True)
        return full_loss
    ...
    def validation_epoch_end(self, outputs):
        ...
        for k, v in flatten_dict(val_evaluator_res).items():
            self.log(f'val_{k}', torch.as_tensor(v))
        ...
        for k, v in flatten_dict(test_evaluator_res).items():
            self.log(f'test_{k}', torch.as_tensor(v))
        ...
        if self.extra_evaluators:
            for cur_eval_title, cur_evaluator in self.extra_evaluators.items():
                ...
                for k, v in flatten_dict(cur_evaluator_res).items():
                    self.log(f'extra_val_{cur_eval_title}_{k}', torch.as_tensor(v))
