...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, activation, use_dropout, conv_kind='default',
        ...
        if padding_type == 'reflect':
            conv_block = conv_block + [nn.ReflectionPad2d(dilation)]
        elif padding_type == 'replicate':
            conv_block = conv_block + [nn.ReplicationPad2d(dilation)]
        ...
        conv_block = conv_block + [conv_layer(in_dim, dim, kernel_size=3, padding=p, dilation=dilation),
                       norm_layer(dim),
                       activation]
        if use_dropout:
        ...
        p = 0
        if padding_type == 'reflect':
            conv_block = conv_block + [nn.ReflectionPad2d(second_dilation)]
        elif padding_type == 'replicate':
            conv_block = conv_block + [nn.ReplicationPad2d(second_dilation)]
        elif padding_type == 'zero':
            p = second_dilation
        else:
            raise NotImplementedError('padding [%s] is not implemented' % padding_type)
        conv_block = conv_block + [conv_layer(dim, dim, kernel_size=3, padding=p, dilation=second_dilation, groups=groups),
                       norm_layer(dim)]
...
class ResnetBlock5x5(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, activation, use_dropout, conv_kind='default',
        ...
        p = 0
        if padding_type == 'reflect':
            conv_block = conv_block + [nn.ReflectionPad2d(dilation * 2)]
        elif padding_type == 'replicate':
            conv_block = conv_block + [nn.ReplicationPad2d(dilation * 2)]
        elif padding_type == 'zero':
            p = dilation * 2
        else:
            raise NotImplementedError('padding [%s] is not implemented' % padding_type)
        ...
        conv_block = conv_block + [conv_layer(in_dim, dim, kernel_size=5, padding=p, dilation=dilation),
                       norm_layer(dim),
                       activation]
        if use_dropout:
            conv_block = conv_block + [nn.Dropout(0.5)]
        ...
        p = 0
        if padding_type == 'reflect':
            conv_block = conv_block + [nn.ReflectionPad2d(second_dilation * 2)]
        elif padding_type == 'replicate':
            conv_block = conv_blovk + [nn.ReplicationPad2d(second_dilation * 2)]
        elif padding_type == 'zero':
            p = second_dilation * 2
        else:
            raise NotImplementedError('padding [%s] is not implemented' % padding_type)
        conv_block = conv_block +  [conv_layer(dim, dim, kernel_size=5, padding=p, dilation=second_dilation, groups=groups),
                       norm_layer(dim)]
...
class MultidilatedResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, conv_layer, norm_layer, activation, use_dropout, dilation=1):
        conv_block = []
        conv_block = conv_block + [conv_layer(dim, dim, kernel_size=3, padding_mode=padding_type),
                       norm_layer(dim),
                       activation]
        if use_dropout:
            conv_block = conv_block + [nn.Dropout(0.5)]
...
class MultiDilatedGlobalGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=3,
        ...
        for i in range(n_downsampling):
            ...
            model = model + [conv_layer(min(max_features, ngf * mult),
        ...
        for i in range(n_blocks):
            if ffc_positions is not None and i in ffc_positions:
                model = model + [FFCResnetBlock(feats_num_bottleneck, padding_type, norm_layer, activation_layer=nn.ReLU,
                                         inline=True, **ffc_kwargs)]
            model = model + [MultidilatedResnetBlock(feats_num_bottleneck, padding_type=padding_type,
        ...
        for i in range(n_downsampling):
            ...
            model = model + deconv_factory(deconv_kind, ngf, mult, up_norm_layer, up_activation, max_features)
            ...
            def make_and_add_blocks(model, block_spec):
                ...
                for i in range(block_spec.n_blocks):
                    ...
                    if resnet_block_kind == "resnetblock5x5":                                            
                        model = model + [ResnetBlock5x5(ngf * mult, padding_type=padding_type, activation=activation, norm_layer=norm_layer,
                                            conv_kind=resnet_conv_kind)]
...
class GlobalGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
        ...
        if dilated_blocks_n_start is not None and dilated_blocks_n_start > 0:
            model = model + make_dil_blocks(dilated_blocks_n_start, dilation_block_kind, dilated_block_kwargs)
        ...
        for i in range(n_blocks):
            ...
            if i == n_blocks // 2 and dilated_blocks_n_middle is not None and dilated_blocks_n_middle > 0:
                model = model + make_dil_blocks(dilated_blocks_n_middle, dilation_block_kind, dilated_block_kwargs)
            ...
            model = model + [ResnetBlock(feats_num_bottleneck, padding_type=padding_type, activation=activation,
        ...
        if dilated_blocks_n is not None and dilated_blocks_n > 0:
            model = model + make_dil_blocks(dilated_blocks_n, dilation_block_kind, dilated_block_kwargs)
...
class GlobalGeneratorFromSuperChannels(nn.Module):
    def __init__(self, input_nc, output_nc, n_downsampling, n_blocks, super_channels, norm_layer="bn", padding_type='reflect', add_out_act=True):
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            model = model + [nn.Conv2d(channels[0+i], channels[1+i], kernel_size=3, stride=2, padding=1, bias=use_bias),
        ...
        for i in range(n_blocks1):
            c = n_downsampling
            dim = channels[c]
            model = model + [ResnetBlock(dim, padding_type=padding_type, norm_layer=norm_layer)]
        ...
        for i in range(n_blocks2):
            ...
            model = model + [ResnetBlock(dim, padding_type=padding_type, norm_layer=norm_layer, **kwargs)]
        ...
        for i in range(n_downsampling):  # add upsampling layers
            mult = 2 ** (n_downsampling - i)
            model = model + [nn.ConvTranspose2d(channels[n_downsampling+3+i],
                      ...
                      nn.ReLU(True)]
        model = model + [nn.ReflectionPad2d(3)]
        model = model + [nn.Conv2d(channels[2*n_downsampling+3], output_nc, kernel_size=7, padding=0)]
