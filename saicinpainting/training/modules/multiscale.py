...
class ResNetHead(nn.Module):
    def __init__(self, input_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
        ...
        for i in range(n_downsampling):
            mult = 2 ** i
            model = model + [conv_layer(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1),
...
class ResNetTail(nn.Module):
    def __init__(self, output_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
        ...
        for _ in range(out_extra_layers_n):
            out_layers = out_layers + [nn.Conv2d(ngf, ngf, kernel_size=1, padding=0),
                           up_norm_layer(ngf),
                           up_activation]
        out_layers = out_layers + [nn.ReflectionPad2d(3),
